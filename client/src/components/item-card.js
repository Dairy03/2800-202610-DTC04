export class DiscountCard {
  ...
  #expiryBadge(days) {
    ...
    if (days <= 0)
      return { label: "Expires today", cls: "bg-red-100 text-red-800" };
    if (days === 1) return { label: "Exp 1d", cls: "bg-red-100 text-red-800" };
    if (days === 2)
      return { label: "Exp 2d", cls: "bg-orange-100 text-orange-800" };
  ...
  #getDiscount() {
    const days = Math.max(
      0,
      Math.round(
        (new Date(this.item.expiry) - new Date()) / (1000 * 60 * 60 * 24),
      ),
    );
    ...
    card.innerHTML = `
            ...
            <div class="flex flex-col items-start ml-4">
                ...
                <p class="text-xs self-start text-gray-400">
                    ${this.item.businessName || "Unknown store"}
                </p>
                <p class="text-xs self-start text-gray-400">
...
export class DiscountList {
  ...
  render() {
    ...
    this.container.className =
      "flex flex-col gap-3 px-2 w-full max-w-md mx-auto";
